...
**Status:** Draft. Reframed by ADR 0001 — this is now a design input for a new app rather
...
Simple decides which checkboxes are ticked by asking whether the stored list contains the
picker's entry — and both sides are a `(label, package)` pair, so the comparison tests the
label as well as the package. An entry is only recognised as selected while its stored label
still matches the system label exactly. The save path then writes back the *current system
labels*, so every edit-and-save round trip overwrites whatever was stored.
...
Simple stores everything as one string: pages joined by `###`, entries within a page joined by
`++`, and each entry's label and package joined by `|`. Nothing is escaped on the way in or
out, and the reader indexes blindly into the split result.

A name containing `|` therefore truncates the name and turns the remainder into the package. A
name containing `++` is worse: "C++" splits the entry in two, and the fragment with no `|`
makes the reader index past the end of its own split, throwing on **every** subsequent load —
the app and the widget both stop working, and the user cannot get back in to undo it. `###`
